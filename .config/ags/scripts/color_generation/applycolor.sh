...
apply_hyprland() {
    # Check if scripts/templates/hypr/a/colors.conf exists
    if [ ! -f "scripts/templates/hypr/a/colors.conf" ]; then
        echo "Template file not found for Hyprland colors. Skipping that."
        return
    fi
    echo "not skipping hyprland"
    # Copy template
    mkdir -p "$CACHE_DIR"/user/generated/hypr/hyprland
    cp "scripts/templates/hypr/a/colors.conf" "$CACHE_DIR"/user/generated/hypr/a/colors.conf
    # Apply colors
    for i in "${!colorlist[@]}"; do
        sed -i "s/{{ ${colorlist[$i]} }}/${colorvalues[$i]#\#}/g" "$CACHE_DIR"/user/generated/hypr/a/colors.conf
    done

    cp "$CACHE_DIR"/user/generated/hypr/a/colors.conf "$XDG_CONFIG_HOME"/hypr/a/colors.conf
}
